...
use DBI;
use htmlmodule::Html;
use dbmodule::SearchDocId;#warum ist das hier nötig und bei dbserver7 nicht?
...
# first create table with docids
    ...
    unless (defined($self->{relevancetable})) {
...
#					      select distinct d.documentid, d.refmanid, a.name, d.year, d.title, j.journaltitle, d.issue, b.booktitle, d.volume, s.seriestitle, d.startpage, d.endpage, d.pdffile, dit.relevance
    ...
    unless ($insertquery = $self->{dbhandle}->prepare(qq/create temporary table $tempouttable
					      ...
					      inner join $self->{relevancetable} dit on (d.documentid = dit.documentid)/)) {
...
# print $self->{sorting};
    ...
    if ($self->{sorting} eq "relevance") {
	$self->{showsort} = "relevance DESC, name, year";
    }
...
# table to store all docids and there relevance
    ...
    &SearchDocId::createrelevancetable ($self->{dbhandle}, $relevancetable) or die "\ncan not build tmp-tabelle\n";
...
=cut
    ...
    &SearchDocId::relortable ($self->{dbhandle}, $relevancetable, $table1);


#    print "<p>@temptablearray</p>";
#    print "<p>calculate relevance</p>";
	...
	if ($connection eq "AND") {
	    &SearchDocId::relandtable ($self->{dbhandle}, $relevancetable, $table1);
	}
...
#    print "<p>look for owner</p>";
	...
	&SearchDocId::replace ($self->{dbhandle}, $ownertemptable, "owner", $ownerquestionstring) or die "\ncan not replace\n";

	&SearchDocId::relandtable ($self->{dbhandle}, $relevancetable, $ownertemptable);
...
#    print "<p>insert into questiontable</p>";
...
#moved down to show what is returned with $self
#    $self->{questionid} = $questionid;
#    $self->{relevancetable} = $relevancetable;


#    my $docidtable = "newtempdocidtable"; #table without relevance-values, containing only docids with values > 0
#    &SearchDocId::createtmptable ($self->{dbhandle}, $docidtable) or die "\ncan not build tmp-tabelle\n";
#    print "<p>createtable</p>";


    $relevancetable = &SearchDocId::delete0relevance ($self->{dbhandle}, $relevancetable); #eigentlich muss Rückgabewert nicht übernommen werden, da ja der Inhalt der Tabelle $relevancetable geändert werden soll 
#    print "<p>withoutrelevance</p>";


    $self->{questionid} = $questionid;
    $self->{relevancetable} = $relevancetable;
    #$self->{docidtable} = $docidtable;
...
sub return_relevancetable {
    my $self = shift;
    if ($self->{relevancetable}) {
	return $self->{relevancetable};
    }
    else {
	return 0;
    }
...
#    }
	...
	&SearchDocId::createrelevancetable ($self->{dbhandle}, $deletedocidtable) or die "\ncan not build tmp-tabelle\n";

	my $doc_ids_string = join("\'\, 1)\,\(\' ", @docidsarray); 
	$doc_ids_string = "\(\'".$doc_ids_string."\'\, 1)";
	my $insertquery;
    
# print "insert into $deletedocidtable \(documentid\) values $doc_ids_string";
    
	unless ($insertquery = $self->{dbhandle}->prepare ("replace $deletedocidtable \(documentid, relevance\) values $doc_ids_string")) {
	    die "replace deletedocidtable prepare error $DBI::err: $DBI::errstr.\n";
	}
#    print "$insertquery\n";
	unless ($insertquery->execute()) {
	    die "replace deletedocidtable execute error $DBI::err: $DBI::errstr.\n";
	}
...
# select only docids from user
	...
	&SearchDocId::replace ($self->{dbhandle}, $deleteownertable, "owner", $ownerquestionstring) or die "\ncan not replace\n";

	&SearchDocId::relandtable ($self->{dbhandle}, $deletedocidtable, $deleteownertable);

	$deletedocidtable = &SearchDocId::delete0relevance ($self->{dbhandle}, $deletedocidtable);
...
#	$deletedocidtable = $deleteownertable;
	$self->{relevancetable} = $deletedocidtable;
